#!/usr/bin/env python3
"""GitHub 可认领 Issue 筛选与打分（v3.1，零第三方依赖，基于共享 github_api 模块）。
...
def score_issue(it, days_active):
    ...
    clarity = 0.0
    clarity += min(len(body.strip()) / 200.0, 1.0) * 12.0          # 描述 >=200 字符满分
    if "```" in body:
        clarity += 8.0
    ...
    s += clarity
    parts.append(f"清晰度{clarity:.0f}/30")
    ...
    if "good first issue" in labels:
        ...
        label_score = 5.0
    s += label_score
    parts.append(f"标签{label_score:.0f}/15")
    ...
    s += c_score
    parts.append(f"评论{c_score:.0f}/15")
    ...
    s += f_score
    parts.append(f"新鲜度{f_score:.0f}/20")

    # 5. Milestone / roadmap 加成（20 分）：被 milestone 关联的 issue 是维护者明确想要的功能，
    #    价值远高于无标签的随机 feature 请求。
    milestone = it.get("milestone")
    if milestone:
        m_score = 15.0
        # 即将到期的 milestone 优先级更高
        ...
        if due:
            try:
                ...
                if 0 <= days_until <= 30:
                    m_score = 20.0  # 本月内到期，最高优先级
                elif days_until < 0:
                    m_score = 12.0  # 已逾期，维护者可能在催
            except Exception:
                pass
        s += m_score
        parts.append(f"milestone{m_score:.0f}/20")
    else:
        parts.append("milestone0/20")

    return round(min(s, 100.0)), parts


def main():
    ap = argparse.ArgumentParser(description="筛选并打分可认领的 Issue（v3.1）")
    ...
    ap.add_argument("--no-fallback", action="store_true", help="禁用全量 open issue fallback（严格标签模式）")
    ...
    # 1b) Fallback：标签搜索零命中时（很多仓库的 roadmap/feature issue 不打标签），
    #     自动拉取全量 open issue 再用打分模型过滤，避免漏检高价值条目。
    if not seen and not args.no_fallback:
        print(f"[信息] 标签搜索零命中，自动 fallback 到全量 open issue 列表...")
        page = 1
        while page <= 3:  # 最多拉 300 条，防止超大仓库拖慢
            ...
            if not isinstance(all_data, list) or not all_data:
                break
            for it in all_data:
                # /repos/issues 端点会把 PR 也混进来，过滤掉 pull_request 字段
